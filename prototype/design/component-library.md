...
组件库采用 **shadcn/ui** 哲学——组件作为**源代码**而非依赖包被添加到项目中，允许开发者完全控制样式和逻辑。这确保了设计与实现的一致性，同时保留了最大程度的定制灵活性。

### 组件设计原则

| 原则 | 描述 | 实践 |
|-----|------|------|
| **可访问性优先** | 每个组件内置 ARIA 支持 | Radix UI 底层、语义化标签 |
| **变体驱动** | 通过 variant/size 控制样式 | Tailwind CSS 工具类 |
| **组合优于继承** | 组件可嵌套组合 | Props 传递children |
| **样式逻辑分离** | 样式与行为解耦 | cn() 工具类合并 |
...
> **设计说明**：按钮是用户执行操作的主要方式。通过变体区分操作优先级，通过尺寸适应不同容器。
...
> **设计说明**：输入框是用户输入数据的主要方式。通过清晰的视觉状态帮助用户理解输入状态。
...
> **设计说明**：卡片是组织和展示内容的基础容器。通过变体区分内容的层级和交互属性。
...
> **设计说明**：模态框用于需要用户专注处理的任务。通过遮罩层和焦点管理确保用户不被干扰。
...
> **设计说明**：标签用于快速传达状态或分类信息。通过颜色和样式区分不同语义。
...
> **设计说明**：Toast 用于非侵入性地反馈操作结果。通过类型区分成功/失败/警告等信息。
...
Success   ┌─────────────────────────────────────────┐
          │◀ ▶ ●▏配置已保存                      ✕│
          └─────────────────────────────────────────┘
            └ green accent border (4px left)

Error     ┌─────────────────────────────────────────┐
          │◀ ▶ ●▏保存失败，请重试                  ✕│
          └─────────────────────────────────────────┘
            └ red error border (4px left)

Warning   ┌─────────────────────────────────────────┐
          │◀ ▶ ●▏配置即将过期                      ✕│
          └─────────────────────────────────────────┘
            └ orange warning border (4px left)
═══════════════════════════════════════════════════════════════════
```
...
> **设计说明**：表单是收集用户输入的核心组件。通过结构化的布局和清晰的验证反馈提升填写效率。
...
Form Layouts
═══════════════════════════════════════════════════════════════════
Single Column                Two Column (Desktop)
┌────────────────────┐       ┌─────────────────┬─────────────────┐
│ Field 1            │       │ Field 1         │ Field 2         │
└────────────────────┘       └─────────────────┴─────────────────┘
┌────────────────────┐       ┌─────────────────┬─────────────────┐
│ Field 2            │       │ Field 3 (full)  │                 │
└────────────────────┘       └─────────────────┴─────────────────┘
═══════════════════════════════════════════════════════════════════
```
...
> **设计说明**：下拉菜单用于从预定义选项中选择。通过触发方式和内容复杂度选择合适的组件。
...
└─────────────────┘  └─────────────────┘  └─────────────────┘
  ...
  (border light)    (primary border + bg)   (border light)
