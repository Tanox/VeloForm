...
Veloform v4.2.0 采用 **工业奢华设计哲学**，融合精密工程的严谨与现代设计的克制。每一个设计决策都服务于功能，通过烧锡色（Burnt Sienna）的单一强调色建立独特的品牌识别。
...
| **无障碍优先** | 尊重用户偏好设置 | prefers-reduced-motion、prefers-color-scheme |
...
**工业奢华色彩策略**：深色系背景 + 烧锡色品牌色 + 语义状态色

- 深灰/黑色背景营造专业、精密的工业氛围
- 烧锡色（Burnt Sienna #e85d2c）作为独特品牌色，避免常见的蓝色/紫色
- 语义色（绿/橙/红）仅在必要时使用
- 支持深色/浅色模式自动切换
...
| Token                    | HEX值 | 语义     | 典型使用场景             |
| ------------------------ | ------------ | -------- | ------------------------ |
| `--bg-primary`           | `#0a0a0b`    | 页面背景 | 页面主背景、滚动区域     |
| `--surface`              | `#141417`    | 表面背景 | 卡片容器、分组背景       |
| `--surface-secondary`    | `#1a1a1e`    | 次级表面 | 输入框背景、悬停状态     |
| `--foreground`           | `#f4f4f5`    | 主文本色 | 主标题、正文内容         |
| `--foreground-secondary` | `#a1a1aa`    | 次要文本 | 副标题、辅助说明         |
| `--border`               | `#2a2a30`    | 默认边框 | 分隔线、卡片描边         |
| `--accent`               | `#e85d2c`    | 品牌主色 | 主要按钮、链接、激活状态 |
| `--accent-hover`         | `#f06b3a`    | 主色悬停 | 主按钮 Hover             |
| `--accent-soft`          | `rgba(232, 93, 44, 0.12)` | 主色浅底 | 选中背景、轻量强调 |
| `--success`              | `#4ade80`    | 成功态   | 成功提示、价格高亮       |
| `--warning`              | `#fbbf24`    | 警告态   | 警告提示                 |
| `--error`                | `#f87171`    | 错误态   | 错误提示、删除操作       |
...
Veloform 使用 **SF Pro** 字体族（Apple 系统字体），确保最佳的渲染效果和性能。无需网络加载，原生支持。
...
| **等宽** | SF Mono        | ui-monospace, 'Menlo', 'Monaco'   | 代码、技术参数        |

### 2.2 CSS 变量定义

```css
--font-sans: -apple-system, BlinkMacSystemFont, 'SF Pro Text', 'SF Pro Display', 'Segoe UI', 'PingFang SC', 'Microsoft YaHei', sans-serif;
...
| 版本   | 日期       | 关键变更                                   |
| ------ | ---------- | ------------------------------------------ |
| v4.2.0 | 2026-07-17 | 工业奢华风格、烧锡色品牌、SF Pro字体、深浅双模式、无障碍支持 |
