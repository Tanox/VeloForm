...
| `ComponentSelectorComponent` | Presentational | 组件选择模态框 | `allComponents`, `currentComponentId`, `bikeType` | `select`, `close` |
| `ConfirmDialogComponent` | Smart | 确认对话框（服务驱动） | — | — |
| `NotificationDisplayComponent` | Smart | 通知显示 | — | — |
| `LoadingIndicatorComponent` | Presentational | 加载指示器 | `isLoading`, `message`, `wrapperClass` | — |

**说明**：
- `ConfirmDialogComponent` 采用服务驱动模式，通过 `ConfirmDialogService` 的 `confirm()` 方法（Promise-based API）调用，而非 input/output 模式。
- `NotificationDisplayComponent` 直接消费 `notificationService` 单例，通过其 `notifications$` signal 渲染通知列表。
- `LoadingIndicatorComponent` 为轻量级展示组件，根据 `isLoading` input 显示/隐藏旋转动画。
