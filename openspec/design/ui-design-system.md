...
本文档定义 DesignHub 项目的 UI 设计系统，包括颜色、字体、间距、组件库等设计规范。设计系统确保应用界面的一致性、可访问性和可扩展性，采用 Apple 设计风格的极简主义原则。
...
DesignHub 采用苹果风格的蓝色作为品牌主色，象征科技、信任与创新。
...
DesignHub 采用 CSS 变量系统实现深色/浅色主题切换，使用 Tailwind CSS 的 `darkMode: 'class'` 配置。
...
.rounded-3xl    { border-radius: 1.5rem; }    /* 24px - 大卡片 */
