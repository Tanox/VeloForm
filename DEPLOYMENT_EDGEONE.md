...
在部署前，请确保以下构建问题已修复（本项目已修复）：

- ✅ Angular 21.x 导入问题：`inject`、`signal`、`computed` 必须显式导入
- ✅ 输出目录路径正确：`dist/browser`

### 2. 必要配置文件

项目已包含以下配置文件：

- `edgeone-pages.config.json` - EdgeOne Pages 专用配置
- `vercel.json` - Vercel 配置（可继续使用）
- `public/_redirects` - SPA 路由重定向
- `public/_headers` - 自定义响应头
...
```json
{
  "framework": "angular",           // 框架类型
  "buildCommand": "npm run build",  // 构建命令
  "outputDirectory": "dist/browser",// 输出目录（关键！）
...
| 输出目录 | `dist/browser` 或 `dist/app/browser` | **必须是 `dist/browser`** |
...
- [本项目 Vercel 部署配置](./openspec/deployment/environments.md)
